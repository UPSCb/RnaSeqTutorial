...
The log files, which contain information relating to the processing and
splice-junctions, are moved into a log directory.

    mkdir star/sample-sortmerna-trimmomatic-STAR\_logs
    mv star/sample-sortmerna-trimmomatic-STARLog.* \
    star/sample-sortmerna-trimmomatic-STARSJ.* \
...
`--pseudobam` tells Kallisto to output a SAM file of pseudoalignments to `stdout`.
...
Kallisto outputs several files:
  ...
  * sample-sortmerna-trimmomatic_pseudo.sam: A SAM file of pseudo-alignments
...
The SAM file can be converted to a sorted BAM and indexed:
    ...
    share/Day1/data/reference/fasta/Ptrichocarpa_v3.0_210_synthetic-gene-models.fa \
    sample-sortmerna-trimmomatic_pseudo.sam | \
    samtools sort -o sample-sortmerna-trimmomatic_pseudo.bam \
    && samtools index sample-sortmerna-trimmomatic_pseudo.bam

```{r head-0606, echo=FALSE}
    h1("Visualising the alignments")
```

Visualising alignments can prove useful to assess the data quality or the aligners
performance. Let us take a look at the STAR results. We already copied them in a
place that JBrowse (the online Genome Browser we will be using, which you can access
from the course webpage, by clicking on your "Connect to Apache2" link) can access.

```{r inv-0604, echo=FALSE}
    endQuest()
```
