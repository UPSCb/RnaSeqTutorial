...
recently sequenced genomes. Another common format is the `gtf` (`Gene Transfer Format`), an alternative to the `gff2` format. This format is arguably popular from its exclusive use by
...
GenomeTools [@Gremme2013] offers a nice utility to validate a gff3 file. 
...
a single read; if it overlaps with a `duplicated` exon, that single molecule will
be counted twice!
...
exons into `super-exons`. We can do that using the Bioconductor easyRNASeq [@easyRNASeq] `createSyntheticTranscripts` function.
...
Let us start with GenomeTools again:
